...
checkboxGroupInput("storegroup",
                    ...
                    choices = list("Farmers Market" = "Farmers Market",
                              ...
                              selected = c("Farmers Market","Food Pantry", "Grocer", "Local foods",
...
# countydata_select<-reactive({
#   countydata_reactive() %>%
#     filter(store_grou %in% input$storegroup)
# })
...
observeEvent(c(input$sour_auth, input$store_grou, input$county, input$loadstores1, input$update),{
 #storepal<-colorFactor(palette="YlOrRd",msadata_reactive()$year_range,reverse=TRUE)
  storepal<-colorFactor(palette="Paired",countydata_reactive()$store_grou,reverse=TRUE)
...
county1data_reactive<-reactive({
  ...
  data<-st_read("data/programloc_190816.gpkg",layer=fips,stringsAsFactors=FALSE) %>%
    mutate(lat=X,long=Y,
      #Define the popup
...
observeEvent(c(input$officegroup, input$county1, input$loadstores2),{
 #storepal<-colorFactor(palette="YlOrRd",msadata_reactive()$year_range,reverse=TRUE)
  ...
  storepal<-colorFactor(topo.colors(7), county1data_reactive()$Type)
