...
checkboxGroupInput("Authorized program", 
                    ("Source"), 
...
output$map1<-renderLeaflet({
  long_max3<-as.vector(st_bbox(cty_boundary2()))[3]+buffer
  latit_max3<-as.vector(st_bbox(cty_boundary2()))[4]+buffer
  long_min3<-as.vector(st_bbox(cty_boundary2()))[1]-buffer
  latit_min3<-as.vector(st_bbox(cty_boundary2()))[2]-buffer

  leaflet(cty_boundary2()) %>%
...
observeEvent(c(input$officegroup, input$county1, input$loadstores2),{
 #storepal<-colorFactor(palette="YlOrRd",msadata_reactive()$year_range,reverse=TRUE)
  ...
  long_max4<-as.vector(st_bbox(cty_boundary2()))[3]+buffer
  latit_max4<-as.vector(st_bbox(cty_boundary2()))[4]+buffer
  long_min4<-as.vector(st_bbox(cty_boundary2()))[1]+buffer
  latit_min4<-as.vector(st_bbox(cty_boundary2()))[2]+buffer
  
 leafletProxy("map1") %>%
    clearMarkers() %>%
    clearControls() %>%
   fitBounds(long_min4,latit_min4,long_max4,latit_max4) %>%
    addPolygons(data=cty_boundary2(),
                fillOpacity=0,
                opacity=0.7,
                stroke=TRUE,
                color="black") %>%
