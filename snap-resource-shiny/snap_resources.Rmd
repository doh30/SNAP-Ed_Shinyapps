...
This map allows you to view SNAP authorized retailers by GA county. Choose the name of the county in the county selection search box and then select the option you want. Then click on the load data button.

Use the tools below to also choose the source of retailers to include (SNAP/WIC). Click on a point to see more information on that retailer.
...
retailersdata<-reactive({
    data<-st_read("data/snap_retailers.gpkg",stringsAsFactors=FALSE)
  })
...
countydata_reactive<-reactive({
  ...
  data<-st_read("data/snap_retailers_usda_ga_ver3.gpkg",layer=fips,stringsAsFactors=FALSE) %>%
    mutate(lat=X,long=Y,
      #Define the popup
...
output$map<-renderLeaflet({
  long_max<-max(countydata_reactive()$X)+buffer
  latit_max<-max(countydata_reactive()$Y)+buffer
  long_min<-min(countydata_reactive()$X)-buffer
  latit_min<-min(countydata_reactive()$Y)-buffer
...
observeEvent(input$county,{
  long_max1<-max(countydata_reactive()$X)+buffer
  latit_max1<-max(countydata_reactive()$Y)+buffer
  long_min1<-min(countydata_reactive()$X)-buffer
  latit_min1<-min(countydata_reactive()$Y)-buffer

  leafletProxy("map") %>%
    fitBounds(long_min1,latit_min1,long_max1,latit_max1)})

observeEvent(c(input$Source, input$store_grou, input$county, input$loadstores1),{
...
This map allows you to view SNAP-Ed program locations by GA county. Choose the name of the county in the county selection search box and then select the option you want. This map includes SNAP-Ed office locations and also SNAP-Ed implementation sites (See the "About this map" page for more details). Then click on the load data button.
...
checkboxGroupInput("officegroup", 
                    ...
                    choices = list("UGA Extension" = "UGA Extension", 
                                   ...
                                   "UGA Site Loc" = "UGA Site Loc",
                                   "Open Hand Atlanta" = "Open Hand Atlanta",
                                   "HealthMPower" = "HealthMPower",
                                   "DPH" = "DPH"),
                              selected = c("UGA Extension","DFCS", "HealthDep", "UGA Site Loc", "Open Hand Atlanta", "HealthMPower", "DPH"))

HTML("If no offices are visible, click on the button below")

actionButton("loadstores2","Load stores")
  ...
  data<-st_read("data/snap_programs_gacounties.gpkg",layer=fips,stringsAsFactors=FALSE) %>%
    mutate(lat=X,long=Y,
      ...
      addr_full=if_else(is.na(Zip),
                         paste("<br><b>",addresses),
                         paste("<br><b>",addresses," ",Zip,sep="")),
      # year_range=case_when(year_first==year_last~as.character(year_first),
      #                      year_first!=year_last~paste(year_first,year_last,sep="-")),
      content=paste("<strong><size=1>",Name,"</strong></size>", 
                       addr_full))
                       # "<br>",city,", ",state,"  ",zip5,"</b>",
                       # "<br><br><b>USDA store classification:</b> ",store_type,
                       # "<br><b>Group category: </b>",store_grou,
                       # "<br><b>Years authorized for SNAP*: </b>",year_range,
                       # "<br>*2008 and 2018 are the first and last years data are available.",sep=""))
  })
...
output$map1<-renderLeaflet({
  long_max<-max(county1data_reactive()$X)+buffer
  latit_max<-max(county1data_reactive()$Y)+buffer
  long_min<-min(county1data_reactive()$X)-buffer
  latit_min<-min(county1data_reactive()$Y)-buffer

  leaflet(county1data_reactive()) %>%
  fitBounds(long_min,latit_min,long_max,latit_max) %>%
   addProviderTiles(providers$CartoDB.Positron) 
})

observeEvent(input$county1,{
  long_max1<-max(county1data_reactive()$X)+buffer
  latit_max1<-max(county1data_reactive()$Y)+buffer
  long_min1<-min(county1data_reactive()$X)-buffer
  latit_min1<-min(county1data_reactive()$Y)-buffer
...
This map allows users to exlore the database of SNAP authorized retailers in 2018 in 159 counties in GA. Users can compare their locations to the WIC authorized retailers by using the "Source" check box in the sidebar. Also, SNAP exclusive/WIC exclusive/both SNAP & WIC authorized details are addressed in the pop-up box when users click on a point. 
    ...
    <strong>SNAP authorized retailers are classified as follows (Store categoty): Farmers Market/Grocer/Local Foods/Small retail/Specialty/Supermarket/Other store.<br>
